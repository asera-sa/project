	...
	var demo5 = function () {
        ...
        L.marker(
            [32.885353,13.180161],
            {
                ...
                title: "Hover Text", // Ad  title
                opacity: 1,
                icon: markerIcon // here assign the markerIcon var
            } // Adjust the opacity
        )
        .addTo(leaflet)
        .bindPopup("<b id = 'popUpAddress'>طرابلس</b>")
        .openPopup();
		// Set Geocoding
		var geocodeService;
		...
		leaflet.on('click', function (e) {
			geocodeService.reverse().latlng(e.latlng).run(function (error, result) {
				...
				$('#address').val( result.address.Match_addr); //ينطبع في انبوت
				$('#popUpAddress').val( result.address.Match_addr); //ينطبع في انبوت
